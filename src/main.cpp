...
bool lineCollision(glm::vec3 p1,glm::vec3 p2,glm::vec3 px);
bool circleCollision(glm::vec3 c,glm::vec3 cam,float r);
void renderQuad();
...
struct ProgramState {
    ...
    glm::vec3 dirLightAmbDiffSpec = glm::vec3(0.05f,0.1f,0.15f);
    bool ImGuiEnabled = false;
    bool ball = false;
    Camera camera;
    bool CameraMouseMovementUpdateEnabled = true;
    glm::vec3 vecCalibrate = glm::vec3(1.0f,0.2f,0.7f);
...
int main() {
    ...
    Shader transparentShader("resources/shaders/mainShader.vs", "resources/shaders/blending.fs");
    ...
    // =======================================[load models]===========================================================
    //garden
    Model gardenModel("resources/objects/garden/StMaryAldermanburyGarden01.obj",true);
    gardenModel.SetShaderTextureNamePrefix("material.");
    ...
    //ball
    Model ballModel("resources/objects/ball/360Background.obj",true);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if(editMode) {
            ...
            programState->camera.Position.y = 0.4f;
        }
        // render
        // ____________________________________________________________________________________
        ...
        ourShader.setVec3("pointLights[0].position", glm::vec3(11.0f+sin(time)*0.5f ,1.0f+sin(time*0.5f)*0.2f, -9.24f+cos(time)*0.5f));
        ...
        ourShader.setVec3("pointLights[1].position", glm::vec3(13.2f,0.2f,4.0f));
        ...
        transparentShader.setVec3("pointLights[0].position", glm::vec3(11.0f+sin(time)*0.5f ,1.4f+sin(time*0.5f)*0.2f, -9.24f+cos(time)*0.5f));
        ...
        transparentShader.setVec3("pointLights[1].position", glm::vec3(13.2f,0.2f,4.0f));
        ...
        //draw skull
        model = glm::mat4(1.0f);
        model = glm::translate(model,glm::vec3(11.0f+sin(time)*0.5f ,1.0f+sin(time*0.5f)*0.2f, -9.24f+cos(time)*0.5f));
        model = glm::scale(model, glm::vec3(0.009f));
        model = glm::rotate(model,glm::radians(-90.0f), glm::vec3(1.0f ,0.0f, 0.0f));
        model = glm::rotate(model,glm::radians(135.0f), glm::vec3(0.0f ,0.0f, 1.0f));
        transparentShader.setMat4("model", model);
        skullModel.Draw(transparentShader);

        //render ball
        model = glm::mat4(1.0f);
        model = glm::translate(model,glm::vec3(13.2f,0.2f+sin(time*3.0f)*0.1f,4.0f));
        model = glm::scale(model, glm::vec3(0.008f));
        model = glm::rotate(model,glm::radians(time*10.0f*sin(time)), glm::vec3(1.0f ,0.0f, 0.0f));
        model = glm::rotate(model,glm::radians(time*12.0f*cos(time)), glm::vec3(0.0f ,1.0f, 0.0f));
        ourShader.setMat4("model", model);
        if(!programState->ball)
            ballModel.Draw(ourShader);
        //ball2
        model = glm::mat4(1.0f);
        model = glm::translate(model,programState->vecCalibrate);
        model = glm::scale(model, glm::vec3(3.0f));
        model = glm::rotate(model,glm::radians(time*2.0f), glm::vec3(1.0f ,0.0f, 0.0f));
        model = glm::rotate(model,glm::radians(time*4.0f), glm::vec3(0.0f ,1.0f, 0.0f));
        model = glm::rotate(model,glm::radians(time*1.2f), glm::vec3(0.0f ,0.0f, 1.0f));
        ourShader.setMat4("model", model);
        if(!circleCollision(glm::vec3(13.2f,0.2f,4.0f),programState->camera.Position,0.2f))
            programState->ball = true;
        if(programState->ball)
            ballModel.Draw(ourShader);
...
bool circleCollision(glm::vec3 c,glm::vec3 x,float r){
    float res;
...
void processInput1(GLFWwindow *window) {
    ...
    if (!(lineCollision(glm::vec3(-1.9f,0.0f,4.0f), glm::vec3(-2.9f,0.0f,-14.2f),programState->camera.Position)) &&
             !(lineCollision(glm::vec3(-1.9f,0.0f,4.0f), glm::vec3(15.2f,0.0f,4.9f),programState->camera.Position)) &&
             (lineCollision(glm::vec3(15.2f,0.0f,4.9f), glm::vec3(14.4f,0.0f,-14.0f),programState->camera.Position)) &&
             (lineCollision(glm::vec3(14.4f,0.0f,-14.0f), glm::vec3(-2.9f,0.0f,-14.2f),programState->camera.Position))
            ) {
...
void DrawImGui(ProgramState *programState) {
    ...
    ImGui::NewFrame();
    {
        ...
        ImGui::DragFloat3("RGB", (float*)&programState->vecCalibrate, 0.05, -300.0, 300.0);
        ImGui::Text(" ");

        ImGui::DragFloat3("DirLight dir", (float*)&programState->dirLightDir, 0.05, -20.0, 20.0);
        ImGui::Text("Ambient    Diffuse    Specular");
        ImGui::DragFloat3("DirLight settings", (float*)&programState->dirLightAmbDiffSpec, 0.05, 0.001, 1.0);
